...
exports.loadCommonFunctions = ({ socket, webMessage }) => {
  ...
  const sendGifFromURL = async (
    ...
    // 1) Preferir reprodução inline usando a URL diretamente
    try {
      return await socket.sendMessage(
        remoteJid,
        {
    ...
    // 2) Se a URL não funcionar, baixar e enviar como vídeo com gifPlayback
    try {
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error(`Falha ao baixar GIF: ${response.status} ${response.statusText}`);
      }
    ...
    // 3) Último fallback: enviar como documento (permitirá download, sem playback)
    try {
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error(`Falha ao baixar GIF: ${response.status} ${response.statusText}`);
    ...
    // 4) Se nada der certo, tenta novamente com a URL simples
    return await socket.sendMessage(
      remoteJid,
      {
        video: { url },
  ...
  const sendGifFromBuffer = async (
    ...
    // 3) Documento como último recurso
    return await socket.sendMessage(
      remoteJid,
      {
        document: buffer,
